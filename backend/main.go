...
func (backend *Backend) InitMaster(plaintext []byte) error {
	// Flow:
	...
	queryResult, err := backend.DB.Exec(
		"INSERT INTO master (password, secret_key, salt, created_at, updated_at) VALUES (?, ?, ?, ?, ?)",
		masterPasswordHashBase64, userSecretKeyEncryptedBase64, saltBase64, now, now)
